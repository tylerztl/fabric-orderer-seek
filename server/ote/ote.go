...
func StartOTE() *OrdererTrafficEngine {
	...
	for _, channel := range AppConf.Channels {
		wg.Add(1)
		go engine.StartConsumer(serverAddr, AppConf.ConnOrderers[0].Host, channel, matches[0], &wg)
	}
	wg.Wait()
	mysql.CloseDB()
	return engine
...
func (e *OrdererTrafficEngine) StartConsumer(serverAddr, serverHost, channelId, cryptoPath string, wg *sync.WaitGroup) {
	defer wg.Done()
	var dialOpts []grpc.DialOption
	dialOpts = append(dialOpts, grpc.WithDefaultCallOptions(grpc.MaxCallSendMsgSize(MaxGrpcMsgSize),
		grpc.MaxCallRecvMsgSize(MaxGrpcMsgSize)))
	...
	if AppConf.SeekPositon == 0 {
		if err = s.seekOldest(); err != nil {
			panic(fmt.Sprintf("Error seek oldest block from serverAddr=%s channelID=%s; err: %v", serverAddr, channelId, err))
		}
	} else {
